...
void main() {
  ...
  test('estimates from the plan\'s equipment mix', () {
    ...
    final plan = _plan(
      sessionLengthMin: 60,
      equipment: const ['Barbell', 'Barbell', 'Bodyweight', 'Bodyweight'],
    );
  ...
  test('Machines, Dumbbells and Kettlebell each contribute MET 5.0', () {
    ...
    for (final equipment in ['Machines', 'Dumbbells', 'Kettlebell']) {
      ...
      expect(
        estimateSessionKcal(plan: plan, weightKg: 70),
        350,
  ...
  test('a band session burns less than a barbell session', () {
    ...
    expect(
      estimateSessionKcal(plan: bands, weightKg: 70),
  ...
  test('returns null without a body weight', () {
    ...
    expect(
      estimateSessionKcal(plan: plan, weightKg: null),
      isNull,
  ...
  test('a plan with no exercises yields null, not a divide by zero', () {
    expect(
      estimateSessionKcal(
        plan: _plan(sessionLengthMin: 60, equipment: const []),
        weightKg: 70,
      ),
      isNull,
    );
