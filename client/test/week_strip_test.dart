...
void main() {
  ...
  testWidgets('a day the user trained fills, whatever day of the week it is', (
    ...
    expect(
      tester.widget<WeekDayCell>(find.byKey(const Key('day.7'))).mark,
      DayMark.trained,
    );
    expect(
      tester.widget<WeekDayCell>(find.byKey(const Key('day.1'))).mark,
  ...
  testWidgets('the strip singles out no weekday, whatever the day count', (
    ...
    expect(
      three[DateTime.wednesday - 1],
      isNot(three[DateTime.thursday - 1]),
  ...
  testWidgets('a week spanning a month boundary keys days by their own month', (
    ...
    final wednesday = tester.widget<WeekDayCell>(
      find.byKey(const Key('day.3')),
    );
  ...
  testWidgets(
    'a week spanning a year boundary walks back into the prior year',
    (tester) async {
      ...
      expect(
        tester.widget<WeekDayCell>(find.byKey(const Key('day.5'))).mark,
        DayMark.trained,
      );
      expect(
        tester.widget<WeekDayCell>(find.byKey(const Key('day.4'))).mark,
        isNot(DayMark.trained),
      );
    },
  );
  ...
  testWidgets('days that are not known claim neither a schedule nor a target', (
    ...
    for (var weekday = 1; weekday <= 7; weekday++) {
      expect(
        markFor(tester, weekday),
        isNot(DayMark.missed),
        reason:
            'day $weekday cannot be missed against a schedule nobody '
            'has managed to read',
      );
    }
  });
  ...
  testWidgets('the card offers Start with no session and Resume with one', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(SessionCard(plan: _plan, hasActiveSession: false, onStart: () {})),
    );
    expect(find.text('Start session'), findsOneWidget);

    await tester.pumpWidget(
      _host(SessionCard(plan: _plan, hasActiveSession: true, onStart: () {})),
    );
  ...
  testWidgets('starting disables the button so it cannot be double-tapped', (
    tester,
  ) async {
    var taps = 0;
    await tester.pumpWidget(
    ...
    await tester.tap(
      find.byKey(const Key('session.start')),
      warnIfMissed: false,
    );
    expect(taps, 0);
  });
