...
--
-- token is the ONLY credential the page has -- there is no sign-in on the
-- reading end -- so it is 32 bytes from a CSPRNG, base64url, never derived
-- from a user id or a timestamp. UNIQUE because a collision would hand one
-- coach another user's report.
--
...
CREATE TABLE IF NOT EXISTS shared_reports (
  ...
  UNIQUE KEY uq_shared_reports_token (token),
