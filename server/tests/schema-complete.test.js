...
const EXPECTED_TABLES = [
  ...
  'progress_reports', 'reminders', 'routine_items', 'session_exercises', 'session_outcomes', 'set_logs',
  'shared_reports', 'streaks',
  'subscriptions', 'user_equipment', 'user_identities', 'user_injuries', 'user_training_days',
  'users', 'workout_plans', 'workout_sessions',
];
...
test('complete FitSync schema', async (t) => {
  ...
  await t.test('creates exactly the 38 tables from the data dictionary', async () => {
  ...
  await t.test('every table has a single-column integer primary key (Table 5)', async () => {
    ...
    assert.equal(rows.length, 39); // 38 tables + schema_migrations
  });
  ...
  await t.test('all twenty-one migrations are recorded', async () => {
    ...
    assert.deepEqual(rows.map((r) => r.version), [
      ...
      '017_plan_source.sql',
      '018_exercise_ai_cues.sql',
      '019_shared_reports.sql',
