...
test('exercise endpoints', async (t) => {
  ...
  await t.test('lists live exercises in the data envelope', async () => {
    const res = await request(app).get('/api/v1/exercises').expect(200);
  ...
  await t.test('filters by muscle group and by equipment', async () => {
    const byMuscle = await request(app).get('/api/v1/exercises?muscleGroup=biceps').expect(200);
    assert.equal(byMuscle.body.data.total, 2);

    const byEquipment = await request(app).get('/api/v1/exercises?equipment=dumbbell').expect(200);
    assert.equal(byEquipment.body.data.total, 1);
  });

  await t.test('rejects a limit above the cap rather than clamping it', async () => {
    const res = await request(app).get('/api/v1/exercises?limit=500').expect(400);
  ...
  await t.test('rejects a non-integer page', async () => {
    const res = await request(app).get('/api/v1/exercises?page=abc').expect(400);
    assert.equal(res.body.error.code, 'INVALID_QUERY_PARAM');
  });
  ...
  await t.test('returns one exercise with cues and an animation', async () => {
    ...
    const res = await request(app).get(`/api/v1/exercises/${exerciseId}`).expect(200);
  ...
  await t.test('an unknown id is a 404 in the error envelope', async () => {
    const res = await request(app).get('/api/v1/exercises/999999').expect(404);
  ...
  await t.test('a pending exercise is a 404, not a leak', async () => {
    ...
    await request(app).get(`/api/v1/exercises/${pending.exercise_id}`).expect(404);
  });

  await t.test('a non-numeric id is a 400, not a 500', async () => {
    const res = await request(app).get('/api/v1/exercises/not-a-number').expect(400);
    assert.equal(res.body.error.code, 'INVALID_QUERY_PARAM');
  });

  await t.test('filters endpoint is not shadowed by the :id route', async () => {
    const res = await request(app).get('/api/v1/exercises/filters').expect(200);
